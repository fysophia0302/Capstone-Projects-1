...
I got a general idea of the data set by _¡®fd.shape¡¯_,  _Df.head()_, _df.info()_ to know the size of the data set, the numbers of columns and so forth.

**4. Drop the unnecessary columns**  
This file contains 25 colomns,  I only kept the useful ones.The purpose of this step is to figure out the columns that are useful for further analysis, as well as to make the data frame easy to run and read.
...
_isnull().sum()_
  ...
  * Second step I clarified the rest columns and fill the missing values with any value we choose, and this is the better techinque to remove missing values from the data set.  

  * For the columns which are numeric I replaced the Null by their mean number.
For ¡®manufacturer¡¯, ¡®model¡¯, ¡®paint_color¡¯ I replaced the null by ¡®unkonwn¡¯ 
